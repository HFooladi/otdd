...
class FSMolDataset(torch.utils.data.Dataset):
    ...
    """
    def __init__(self, data, name, featurizer='desc2D', split='train', transform=None, target_transform=None):
        self.data_path = data
        self.name = name
...
def load_fsmol_data(dataname: str, task='test', split='train', featurizer='desc2D', batch_size = 64, shuffle=True, transform=None, target_transform=None):
    ...
    Arguments:
        dataname (str): name of dataset (ChEMBL assay id)
        task (str): task to use (choose from 'test', 'train')
        split (str): split to use
        featurizer (Featurizer): faeturizer to use
        batch_size (int): batch size
        shuffle (bool): whether to shuffle data
    ...
    Returns:
        ...
        train (FSMolDataset): train dataset
        test (FSMolDataset): test dataset

    """
    data_path = os.path.join(DATA_DIR, f'fsmol_{featurizer}_{task}.pkl')
    train = FSMolDataset(data_path, dataname, split='train', featurizer=featurizer)
    #test = FSMolDataset(data_path, dataname, split='test', featurizer=featurizer)
    train_loader = dataloader.DataLoader(train, batch_size=batch_size, shuffle=shuffle)
    #valid_loader = dataloader.DataLoader(train, batch_size=32, shuffle=True)
    #test_loader = dataloader.DataLoader(test, batch_size=32, shuffle=True)
